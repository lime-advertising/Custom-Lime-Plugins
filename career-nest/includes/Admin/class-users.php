...
class Users
{
    ...
    public function render_employer_field(\WP_User $user): void
    {
        ...
        $job_title = get_user_meta($user->ID, '_job_title', true);
        $personal_phone = get_user_meta($user->ID, '_personal_phone', true);
        $bio = get_user_meta($user->ID, '_bio', true);
        ...
        // Personal Profile Fields for Employer Team Members
        echo '<div id="careernest-personal-profile-wrap-edit" style="' . ($is_team ? '' : 'display:none;') . '">';
        echo '<h2>' . esc_html__('CareerNest — Personal Profile', 'careernest') . '</h2>';
        echo '<table class="form-table" role="presentation">';

        echo '<tr><th><label for="careernest_job_title">' . esc_html__('Job Title', 'careernest') . '</label></th><td>';
        echo '<input type="text" id="careernest_job_title" name="careernest_job_title" class="regular-text" value="' . esc_attr($job_title) . '" placeholder="e.g., HR Manager, Recruiter" />';
        echo '<p class="description">' . esc_html__('The user\'s job title within the company.', 'careernest') . '</p>';
        echo '</td></tr>';

        echo '<tr><th><label for="careernest_personal_phone">' . esc_html__('Phone Number', 'careernest') . '</label></th><td>';
        echo '<input type="tel" id="careernest_personal_phone" name="careernest_personal_phone" class="regular-text" value="' . esc_attr($personal_phone) . '" />';
        echo '<p class="description">' . esc_html__('Personal phone number for the team member.', 'careernest') . '</p>';
        echo '</td></tr>';

        echo '<tr><th><label for="careernest_bio">' . esc_html__('Bio', 'careernest') . '</label></th><td>';
        echo '<textarea id="careernest_bio" name="careernest_bio" rows="5" class="large-text" placeholder="Tell us about yourself and your role...">' . esc_textarea($bio) . '</textarea>';
        echo '<p class="description">' . esc_html__('Personal bio/description for the team member.', 'careernest') . '</p>';
        echo '</td></tr>';
        ...
        echo '<script>(function(){
          var wrapLink = document.getElementById("careernest-employer-link-wrap-edit");
          var wrapProfile = document.getElementById("careernest-personal-profile-wrap-edit");
          var role = document.getElementById("role") || document.querySelector("select[name=role]");
          function update(){ 
            if(!role||!wrapLink||!wrapProfile) return; 
            var isTeam = (role.value === "employer_team");
            wrapLink.style.display = isTeam ? "block" : "none"; 
            wrapProfile.style.display = isTeam ? "block" : "none"; 
          }
    ...
    public function save_employer_field(int $user_id): void
    {
        ...
        if ($user && in_array('employer_team', (array) $user->roles, true)) {
            ...
            $this->persist_user_personal_profile($user_id);
        }
    }
    ...
    private function persist_user_personal_profile(int $user_id): void
    {
        // Save job title
        $job_title = isset($_POST['careernest_job_title']) ? sanitize_text_field(wp_unslash($_POST['careernest_job_title'])) : '';
        if ($job_title !== '') {
            update_user_meta($user_id, '_job_title', $job_title);
        } else {
            delete_user_meta($user_id, '_job_title');
        }

        // Save personal phone
        $personal_phone = isset($_POST['careernest_personal_phone']) ? sanitize_text_field(wp_unslash($_POST['careernest_personal_phone'])) : '';
        if ($personal_phone !== '') {
            update_user_meta($user_id, '_personal_phone', $personal_phone);
        } else {
            delete_user_meta($user_id, '_personal_phone');
        }

        // Save bio
        $bio = isset($_POST['careernest_bio']) ? wp_kses_post(wp_unslash($_POST['careernest_bio'])) : '';
        if ($bio !== '') {
            update_user_meta($user_id, '_bio', $bio);
        } else {
            delete_user_meta($user_id, '_bio');
        }
    }
}
