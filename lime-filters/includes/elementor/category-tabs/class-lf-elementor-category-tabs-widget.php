...
<?php
    ...
    protected function render_product_card($product_id) {
        ...
        if ($this->supports_add_to_cart($product)) {
            $button = $this->get_add_to_cart_button($product);
            if ($button) {
                $actions[] = $button;
            }
        } else {
            $compare = $this->get_compare_button($product);
            if ($compare) {
                $actions[] = $compare;
            }
        }
...
<?php
    ...
    protected function get_category_links($product_id) {
    ...
    protected function supports_add_to_cart($product) {
        ...
        $slugs = $this->get_product_category_slugs($product->get_id());
        if (empty($slugs)) {
            return false;
        }
    ...
    protected function get_add_to_cart_button($product) {
        if (! $product instanceof WC_Product || ! function_exists('woocommerce_template_loop_add_to_cart')) {
            return '';
        }
        ...
        if ($count === 0) {
            $button = preg_replace(
                '/<a /',
                '<a class="lf-button lf-button--primary" ',
                $button,
                1,
                $count
            );
    ...
    protected function get_compare_button($product) {
        if (! $product instanceof WC_Product) {
            return '';
        }
        ...
        $blocked = apply_filters('lime_filters_compare_blocked_categories', ['accessories', 'parts']);
        $slugs = $this->get_product_category_slugs($product_id);
        if (!empty($blocked) && array_intersect($blocked, $slugs)) {
            return '';
        }
    ...
    protected function get_product_category_slugs($product_id) {
